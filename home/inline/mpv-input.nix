# Verbatim content of mpv/input.conf, wrapped as a Nix indented string (no clean
# structured-format match -- see chat, 2026-09-01) rather than manually
# retyped, to guarantee byte-for-byte fidelity.
''
WHEEL_UP      add volume 2
WHEEL_DOWN    add volume -2
UP            add volume 2
DOWN          add volume -2
AXIS_UP       add volume 2
AXIS_DOWN     add volume -2

Ctrl+RIGHT    seek 85 exact
Ctrl+LEFT     seek -85 exact
Shift+RIGHT   frame-step
Shift+LEFT    frame-back-step

v             cycle deband
a             cycle audio
s             cycle sub
i             cycle interpolation
t             script-message-to seek_to toggle-seeker

+             add audio-delay 0.010
-             add audio-delay -0.010
F1            add sub-delay -0.1
F2            add sub-delay +0.1 
F4            cycle-values video-aspect-override "16:9" "4:3" "2.35:1" "-1"

# Zoom Actions
-             add video-zoom -.25
+             add video-zoom .25

kp8           add video-pan-y .05
kp6           add video-pan-x -.05
kp2           add video-pan-y -.05
kp4           add video-pan-x .05
kp5           set video-pan-x 0; set video-pan-y 0; set video-zoom 0

p             script-binding playlistmanager/showplaylist
''
